...
def longshot_judgement(prediction, realtime):
    ...
    base = {
        ...
        "time_rank": time_rank, "deadline": None, "remaining_minutes": None,
    }
    ...
    model_probability = float(candidate.get("relative_probability") or 0)
    status = "UP" if time_rank <= 2 else "WATCH"
    message = "展示2位以内のため穴候補（オッズ非反映）" if status == "UP" else "展示順位が基準未満のため監視"
    return {
        **base, "status": status, "min_odds": None,
        "model_probability": round(model_probability, 2),
        "market_probability": None, "net_edge": None,
        "message": message,
    }
